...
static QUIC_STATUS QUIC_API SubscriberStreamCallback(
    HQUIC StreamHandle,
    void* Context,
...
static QUIC_STATUS QUIC_API SubscriberConnectionCallback(
    ...
    switch (Event->Type) {
        case QUIC_CONNECTION_EVENT_CONNECTED: {
            printf("[SUBSCRIBER] Conectado al broker QUIC\n");
            fflush(stdout);
            g_connected = 1;
...
int main(int argc, char** argv) {
    ...
    fflush(stdout);
    if (!fgets(g_topic, sizeof(g_topic), stdin)) return 0;
    g_topic[strcspn(g_topic, "\r\n")] = '\0';

    QUIC_STATUS st = MsQuicOpen2(&MsQuic);
    if (QUIC_FAILED(st)) die("MsQuicOpen2 fallo", st);

    QUIC_REGISTRATION_CONFIG reg_cfg = { "subscriber_quic", QUIC_EXECUTION_PROFILE_LOW_LATENCY };
